...
| textlint                 | one analysis per document shared by every rule with no rule-specific options of its own; preset of independent rule modules; shared-config resolution with key-by-key option layering                                                                                   |
