...
export type ProtectedPatternRejectionReason =
  ...
  | 'source-too-long'
  /** Community ReDoS analysis found super-linear backtracking beyond the accepted baseline. */
  | 'unsafe-complexity'
  /** Community ReDoS analysis could not determine whether the source is safe. */
...
export function screenExtraPatterns(sources: readonly string[]): ScreenedProtectedPatterns {
  ...
  for (const source of sources) {
    ...
    if (safety.status === 'vulnerable') {
      rejected.push({
        source,
        reason: 'unsafe-complexity',
        explanation: safety.explanation,
      });
      continue;
    }
    if (safety.matchesOnlyEmpty) {
      rejected.push({
        source,
